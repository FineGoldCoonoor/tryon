// --- Mirror Mode with Eased Jewelry Movement ---
...
faceMesh.onResults((results) => {
  ...
  if (results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
    ...
    if (!smoothedLandmarks) {
      smoothedLandmarks = newLandmarks.map(pt => ({ ...pt }));
      velocityLandmarks = newLandmarks.map(() => ({ x: 0, y: 0, z: 0 }));
    } else {
      smoothedLandmarks = smoothedLandmarks.map((prev, i) => {
        const target = newLandmarks[i];
        velocityLandmarks[i].x += (target.x - prev.x) * easeFactor;
        velocityLandmarks[i].y += (target.y - prev.y) * easeFactor;
        velocityLandmarks[i].z += (target.z - prev.z) * easeFactor;

        return {
          x: prev.x + velocityLandmarks[i].x,
          y: prev.y + velocityLandmarks[i].y,
          z: prev.z + velocityLandmarks[i].z,
        };
      });
    }
